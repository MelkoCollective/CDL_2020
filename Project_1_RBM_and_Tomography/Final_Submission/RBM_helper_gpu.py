...
class RBM():
    ...
    def initialize_parameters(self):
        self.weights = torch.randn(
            ...
            self.n_vis, 
            dtype=torch.double, 
        device = device) / np.sqrt(self.n_vis)
        
        self.visible_bias = torch.zeros(self.n_vis, dtype=torch.double, device = device)
        self.hidden_bias = torch.zeros(self.n_hin, dtype=torch.double, device = device)
